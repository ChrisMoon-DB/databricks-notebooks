...
# MAGIC
# MAGIC **TRY_ABAC**: Set to `True` to execute actual ABAC commands (requires Beta features, creates permanent tables)
# MAGIC
# MAGIC **ABAC_TABLE**: Fully qualified table name for ABAC demonstrations (e.g., `catalog.schema.table`)
# MAGIC
# MAGIC **ABAC_VIEW**: Fully qualified view name for ABAC demonstrations
...
ABAC_TABLE = "dw_serverless_ws1.default.employee_info"  # Fully qualified table name for ABAC tagging
ABAC_VIEW = "dw_serverless_ws1.default.v_employee_info_abac"  # Fully qualified view name for ABAC demo
...
# ABAC permanent table (only if TRY_ABAC = True)
if TRY_ABAC:
    print(f"\n→ Creating ABAC permanent table: {ABAC_TABLE}")
    
    try:
        # Extract catalog and schema from fully qualified table name
        parts = ABAC_TABLE.split('.')
        if len(parts) == 3:
            ...
            # Create and populate the ABAC table
            spark.sql(f"""
                CREATE OR REPLACE TABLE {ABAC_TABLE} (
                    id INT, 
                    name STRING, 
                    salary DECIMAL(10, 2), 
                    ssn STRING
            ...
            print(f"✓ ABAC table {ABAC_TABLE} created and populated")
        else:
            print(f"⚠️  Invalid ABAC_TABLE format (expected: catalog.schema.table)")
            
    except Exception as e:
        print(f"⚠️  Failed to create ABAC table: {e}")
...
if TRY_ABAC:
    print(f"→ ABAC permanent table ready: {ABAC_TABLE}")
...
RETURN CASE
    ...
    WHEN IS_ACCOUNT_GROUP_MEMBER('CAN_SEE_EU') AND region = 'EU' THEN original
    ELSE token
END""")
...
if TRY_ABAC:
    ...
    try:
        ...
        # Step 4: Remove user from the view's ABAC whitelist and re-run
        spark.sql(f"""CREATE OR REPLACE VIEW {ABAC_VIEW} AS
        SELECT 
            id, 
            name, 
        ...
        print("✓ ABAC tagging applied successfully!")
        print("✓ View created with dynamic user-based masking")
        print(f"\n✓ You ({current_user}) will NOT see full SSN values")
        print("✓ You will see masked values\n")
        ...
        print("\n→ Setting TRY_ABAC = False to use simulation mode")
        TRY_ABAC = False
...
if not TRY_ABAC:
    # Simulating ABAC controls since ABAC is not enabled
...
encryption_key = "MySecureKey12345"  # ⚠️ Use Databricks Secrets or a Keyvault in production

print("Unencrypted Data")
display(spark.sql("SELECT * FROM employee_info"))
...
SELECT id, name, salary,
    CASE
        WHEN (CURRENT_USER() = '{current_user}' OR IS_ACCOUNT_GROUP_MEMBER('hr_admin')) THEN CAST(aes_decrypt(unbase64(ssn_encrypted), '{encryption_key}', 'ECB', 'PKCS') AS STRING)
        ELSE '***-**-****'
    END AS ssn
...
# MAGIC ## Cleanup
# MAGIC
# MAGIC **Note:** All temporary tables, views, and functions are automatically cleaned up when your Spark session ends.
# MAGIC
# MAGIC If you enabled `TRY_ABAC = True`, you may want to clean up the permanent ABAC table and view.
...
if TRY_ABAC:
    print("\n⚠️  ABAC was enabled - you created permanent objects:")
    print(f"   • Table: {ABAC_TABLE}")
    print(f"   • View: {ABAC_VIEW}")
    print("\nTo clean up ABAC objects, run the following commands:")
    print(f"   DROP VIEW IF EXISTS {ABAC_VIEW};")
    print(f"   DROP TABLE IF EXISTS {ABAC_TABLE};")
    print("\nOr uncomment and run the cleanup code below:\n")
    
    # Uncomment to automatically cleanup ABAC objects
    # try:
    #     spark.sql(f"DROP VIEW IF EXISTS {ABAC_VIEW}")
    #     print(f"✓ Dropped view {ABAC_VIEW}")
    # except Exception as e:
    #     print(f"⚠️  Could not drop view: {e}")
    #     
    # try:
    #     spark.sql(f"DROP TABLE IF EXISTS {ABAC_TABLE}")
    #     print(f"✓ Dropped table {ABAC_TABLE}")
    # except Exception as e:
    #     print(f"⚠️  Could not drop table: {e}")
    
else:
    print("\n✓ Using temporary tables, views, and functions only")
